...
export async function leaguesRoutes(fastify) {
    // -------------------------------------------------------------
    // ROUTE 1: GET /leagues
    // -------------------------------------------------------------
    fastify.get('/leagues', {
    ...
    // -------------------------------------------------------------
    fastify.get('/leagues/:leagueId/dates', async (request, reply) => {
        ...
        try {
            ...
            const tableData = games.flatMap((game) => game.entries.map((entry) => ({
                date: game.gameDate,
    ...
    // -------------------------------------------------------------
    fastify.get('/leagues/:leagueId/summary', async (request, reply) => {
        const { leagueId } = request.params;
